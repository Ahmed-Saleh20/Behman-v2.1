...
<div class="users_data" >
		<table>
			...
			<form method='post'>
			<?php
			$user_data = $con->prepare("SELECT * FROM users WHERE type='2'and user_block='1'");
		$user_data ->execute();
		...
		foreach ($posts as $key => $post)
		{
	        ...
	        echo "<td>".$email."</td>";
	        echo "<td>".$password."</td>";
		    echo "<td>"."<button id='b3' name='unb".$id."' class='users_data_button1' >un block</button> "."</td>";
		    
		    echo "</tr> ";
		    echo "</tbody>";
		    if (isset($_POST['unb'.$id])) {
              	 
                    $user_data2 = $con->prepare("UPDATE `users` SET `user_block`=0 WHERE user_id =$id ");
					$user_data2 ->execute();
